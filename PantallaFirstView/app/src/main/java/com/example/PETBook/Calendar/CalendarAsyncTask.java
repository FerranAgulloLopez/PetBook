...
abstract class CalendarAsyncTask extends AsyncTask<Void, Void, Boolean> {
  ...
  protected final Boolean doInBackground(Void... ignored) {
    try {
      ...
      activity.startActivityForResult(
          userRecoverableException.getIntent(), CalendarSync.REQUEST_AUTHORIZATION);
    } catch (IOException e) {
      Utils.logAndShow(activity, CalendarSync.TAG, e);
    }
    return false;
  }
  ...
  protected final void onPostExecute(Boolean success) {
    ...
    if (success) {
      activity.refreshView(); // Refresca la vista, ahora el codigo no hace nada(esta comentado con /* */, sin embargo aqui podria ir un mensaje de "Sincronizado bien, mal, etc...
    }
  }
